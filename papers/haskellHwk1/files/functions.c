...
* as an input char is expected and according to his case
* will be this letter changed to another case
...
* (there is a control if parameters are > 0) are expected and returned value is also integer
...
* if first argument plus third argument are higher than second argument, function returns first argument
* because second argument is border of range
* else if first argument plus third argument are equal to second argument, function returns first argument
* plus second argument (it is also possible return twice first argument plus third argument)
* else if first plus third argument is lower than second argument, we add first argument to recursion call
* where first argument is previous first argument plus added range ( = third argument ), other parameters are same*/
int addRange(int a, int b, int c)
{
      int error= 0;
      if(a > b) 
...
/*function 'areaTriangle' expects three Floats which are length of concrete side of triangle
